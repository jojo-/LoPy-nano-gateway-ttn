...
class NanoGateway:
    ...
    def _make_node_packet(self, rx_data, rx_time, tmst, sf, rssi, snr):
        ...
        print('data received from node: ' + str(RX_PK["rxpk"][0]["data"]))
    ...
    def _ack_pull_rsp(self, token, error):
        ...
        with self.udp_lock:
            try:
                print("Sending ack pull rsp")
                self.sock.sendto(packet, self.server_ip)
            except Exception:
                print("PULL RSP ACK exception")

    def _lora_cb(self, lora):
        print('In lora callback')
        events = lora.events()
    ...
    def _send_down_link(self, data, tmst, datarate, frequency):
        self.lora.init(mode=LoRa.LORA, frequency=frequency, bandwidth=LoRa.BW_500KHZ,
